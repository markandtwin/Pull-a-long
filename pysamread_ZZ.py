...
parser.add_argument('-r','--rest', dest='r', action='store', default='rest.bam', help='path to failed bam file')
parser.add_argument('-b','--bed', dest='b', action='store', required=True, help='bed file of filtering region')
parser.add_argument('-t','--nthread', dest='b', action='store', default='4', help='number of threads')
args = parser.parse_args()


# def input_init():
#     global firstindex, lastindex, inputFileName, outputFileName
#     inputFileName = input("Please enter the input file name: ")
#     outputFileName = input("Please enter the output file name: ")
#     firstindex =int(input("Please input the first index: "))
#     lastindex = int(input("Please input the last index: "))
...
def read_bam_file(cname,firstindex,lastindex,inputFileName, outputFileName_valid, outputFileName_invalid):
    ...
    print("valid lines: ", (counter - invalidNums))
...
def composed_func(cname,firstindex,lastindex,inputFileName, outputFileName_valid, outputFileName_invalid):
    ...
    caculate()
    read_bam_file(cname,firstindex,lastindex,inputFileName, outputFileName_valid, outputFileName_invalid)
    pre_index.clear()
    post_index.clear()
    counter =0
    sum.clear()
...
def read_bed_file(inputFileName, outputFileName_valid, outputFileName_invalid, bedFileName):
    ...
    for line in lines:
        ...
        composed_func(chname, first, final, inputFileName, outputFileName_valid_num, outputFileName_invalid_num)
        fileNum += 1
...
def output_composed_bam(inputFileName,outputFileName_valid, outputFileName_invalid):
    ...
    samfile=pysam.AlignmentFile(inputFileName,'rb')
    with pysam.AlignmentFile(outputFileName_valid, "wb", template=samfile) as oct1file:
        for ps in deal_all_valid:
            oct1file.write(ps)
    #subprocess.call('samtools','sort', )

    with pysam.AlignmentFile(outputFileName_invalid, "wb", template=samfile) as oct2file:
...
if __name__=="__main__":
    ...
    # the command line of implementation is as below: 
    # you should replace input_bam, output_bam_valid, output_bam_invalid, bed_filename with the corresponding file name
    # python3 pysamread.py  input_bam  output_bam_valid  output_bam_invalid   bed_filename
